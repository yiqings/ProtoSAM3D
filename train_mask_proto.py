...
from segment_anything.utils.module import momentum_update,l2_normalize,distributed_sinkhorn
...
class BaseTrainer:
    def __init__(self, model, dataloaders, args):
        ...
        patch_size = 4
        in_channels = (384+384+48)*8*8*8
        self.num_classes = 50
        self.num_prototype = 15
        
        self.mask_proto = nn.ModuleDict()
        self.mask_proto.update(
            {"patch_embed": PatchEmbed3D(kernel_size=(patch_size, patch_size, patch_size),
                                         stride=(patch_size, patch_size, patch_size),
                                         in_chans=48,
                                         embed_dim=48)})

        self.mask_proto.update({"blocks":nn.ModuleList()})
        for i in range(9):
            block = Block3D(
                dim=384+384+48,
                num_heads=12,
            ...
            self.mask_proto["blocks"].append(block)
            

        self.prototypes = nn.Parameter(torch.zeros(self.num_classes, self.num_prototype, in_channels),requires_grad=True).to(device)

        # self.mask_proto.update({"channel_proj":nn.Conv3d(384+384+48, self.num_classes, 1),})
        self.mask_proto.update({"feat_norm": nn.LayerNorm(in_channels)})
        self.mask_proto.update({"mask_norm": nn.LayerNorm(self.num_classes)})
        self.mask_proto.to(device)
    ...
    def set_loss_fn(self):
        ...
        self.mask_loss = PixelPrototypeCELoss(configer)
    ...
    def save_checkpoint(self, epoch, state_dict, describe="last"):
        torch.save({
            ...
            "mask_proto":self.mask_proto.state_dict(),
    ...
    def interaction(self, sam_model, image_embedding, gt3D, gt_mask_class, num_clicks):
        return_loss = 0
        ...
        upscaled_embedding_mask_patch = self.mask_proto["patch_embed"](upscaled_embedding_mask)
        # # 创建没有和mask点乘的featur map
        # upscaled_embedding_mask = self.mask_proto["patch_embed"](upscaled_embedding)
        ...
        for blk in self.mask_proto["blocks"]:
        ...
        _c = self.mask_proto["feat_norm"](_c)
        _c = l2_normalize(_c)
        ...
        out_seg = self.mask_proto["mask_norm"](out_seg)
        out_seg = rearrange(out_seg, "(b d) k -> b k d", b=image_embedding.shape[0], k=self.num_classes)

        if sam_model.training:
            # gt_seg = torch.full((image_embedding.shape[0],out_seg.shape[1]), 0, dtype=torch.float, device=out_seg.device)
            # for i in range(image_embedding.shape[0]):
            #     gt_seg[i,gt_mask_class.values[i]] = gt_mask_class.values[i]
            gt_seg = gt_mask_class.values-1
            ...
            mask_loss = self.mask_loss(output,gt_seg.unsqueeze(1))
            return_loss += mask_loss
        ...
        for step, (image3D, gt3D) in enumerate(tbar):
            ...
            with my_context():
                ...
                with amp.autocast():
                    ...
                    prev_masks, loss, out_seg = self.interaction(sam_model, image_embedding, gt3D, gt_mask_class, num_clicks=11)                

                epoch_loss += loss.item()
                epoch_accuracy += (out_seg.squeeze(2).argmax(1)==gt_mask_class.values-1).sum()/gt3D.shape[0]
            ...
            if step % self.args.accumulation_steps == 0 and step != 0:
                ...
                print_accuracy = (out_seg.squeeze(2).argmax(1)==gt_mask_class.values-1).sum()/gt3D.shape[0]
            else:
                step_loss += cur_loss
