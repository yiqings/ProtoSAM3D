...
def finetune_model_predict3D(img3D, gt3D, sam_model_tune, cls_head_3d,proj_head_3d,feat_norm,mask_norm, prototypes, device='cuda', click_method='random', num_clicks=10, prev_masks=None):
    ...
    for num_click in range(num_clicks):
        with torch.no_grad():
            ...
            prev_masks_class = prev_masks*out_seg.argmax(1)
            ...
            iou_list.append(round(compute_iou(medsam_seg, gt3D[0][0].detach().cpu().numpy()), 4))
            dice_list.append(round(compute_dice(gt3D[0][0].detach().cpu().numpy().astype(np.uint8), medsam_seg), 4))
            accuracy_list.append((prev_masks_class.reshape(gt3D.shape[0],-1).max(1)[0]==gt_mask_class.values).sum()/gt3D.shape[0])
            
    return pred_list, click_points, click_labels, iou_list, dice_list,accuracy_list
...
if __name__ == "__main__":    
    ...
    if(args.dim==3):
        ...
        num_classes = 117
        num_prototype = 15
        configer = Configer(configs='/media/barry/SELF/DeepEyes_Project/20240104/SAM-Med3D/protoseg.json')
        cls_head_3d = nn.Sequential(
            nn.Conv3d(in_channels, in_channels, kernel_size=3, stride=1, padding=1),
            BNReLU(in_channels, bn_type=configer.get('network', 'bn_type')),
            nn.Dropout3d(0.10)
        ).to(device)

        prototypes = nn.Parameter(torch.zeros(num_classes, num_prototype, in_channels),requires_grad=True).to(device)

        proj_head_3d = ProjectionHead_3D(in_channels, in_channels).to(device)
        feat_norm = nn.LayerNorm(in_channels).to(device)
        mask_norm = nn.LayerNorm(num_classes).to(device)
        sam_model_tune = sam_model_registry3D[args.model_type](checkpoint=None).to(device)
        if checkpoint_path is not None:
            model_dict = torch.load(checkpoint_path, map_location=device)
            state_dict = model_dict['model_state_dict']
            cls_head_3d_state_dict = model_dict["cls_head_3d"]
            proj_head_3d_state_dict = model_dict["proj_head_3d"]
            feat_norm_state_dict = model_dict["feat_norm"]
            mask_norm_state_dict = model_dict["mask_norm"]
            prototypes = model_dict["prototypes"]
            cls_head_3d.load_state_dict(cls_head_3d_state_dict)
            proj_head_3d.load_state_dict(proj_head_3d_state_dict)
            feat_norm.load_state_dict(feat_norm_state_dict)
            mask_norm.load_state_dict(mask_norm_state_dict)
    ...
    for batch_data in tqdm(test_dataloader):
        ...
        if(os.path.exists(pred_path)):
            ...
            if(args.dim==3):
                seg_mask_list, points, labels, iou_list, dice_list,accuracy_list = finetune_model_predict3D(
                    image3D, gt3D, sam_model_tune, cls_head_3d,proj_head_3d,feat_norm,mask_norm, prototypes, device=device, 
        ...
        all_accuracy_list += accuracy_list[0]
        out_accuracy[img_name] = accuracy_list[0]
