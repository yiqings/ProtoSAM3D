...
from segment_anything.utils.module import l2_normalize
...
def finetune_model_predict3D(img3D, gt3D, sam_model_tune, mask_proto, prototypes, device='cuda', click_method='random', num_clicks=10, prev_masks=None):
    img3D = norm_transform(img3D.squeeze(dim=1)) # (N, C, W, H, D)
    img3D = img3D.unsqueeze(dim=1)
    # 把gt中的标签和mask分离
    gt_mask_class = gt3D.reshape(image3D.shape[0],-1).max(1).values.to(device)
    gt3D[gt3D != 0] = 1
    ...
    for num_click in range(num_clicks):
        with torch.no_grad():
            ...
            low_res_masks, _, src, upscaled_embedding = sam_model_tune.mask_decoder(
            ...
            mask_factor = low_res_masks
            mask_factor_downsampled = F.interpolate(low_res_masks.float(), size=image_embedding.shape[-3:])
            
            image_embedding_mask = torch.mul(mask_factor_downsampled,image_embedding)
            src_mask = torch.mul(mask_factor_downsampled,src)
            # upscaled_embedding_mask = torch.mul(mask_factor,upscaled_embedding)
            # upscaled_embedding_mask_patch = mask_proto["patch_embed"](upscaled_embedding_mask)
            # 创建没有和mask点乘的featur map
            upscaled_embedding_mask = mask_proto["patch_embed"](upscaled_embedding)
            upscaled_embedding_mask_patch = torch.mul(mask_factor_downsampled.permute(0, 2, 3, 4, 1),upscaled_embedding_mask)
            
            # mask_input = torch.concat((image_embedding_mask.permute(0, 2, 3, 4, 1),src_mask.permute(0, 2, 3, 4, 1),upscaled_embedding_mask_patch,
                                   # image_embedding.permute(0, 2, 3, 4, 1),src.permute(0, 2, 3, 4, 1),upscaled_embedding_mask),dim=4)
            mask_input = torch.concat((image_embedding_mask.permute(0, 2, 3, 4, 1),src_mask.permute(0, 2, 3, 4, 1),upscaled_embedding_mask_patch),dim=4)
            
            for blk in mask_proto["blocks"]:
                mask_input = blk(mask_input)
            
            # _c = self.mask_proto["channel_proj"](mask_input.permute(0, 4, 1, 2, 3))
            _c = rearrange(mask_input, 'b c d h w -> b (c d h w)')
            _c = mask_proto["feat_norm"](_c)
            _c = l2_normalize(_c)
            ...
            out_seg = mask_proto["mask_norm"](out_seg)
            out_seg = rearrange(out_seg, "(b d) k -> b k d", b=image_embedding.shape[0], d=1)
            class_list.append(out_seg.argmax(1))
            ...
            iou_list.append(round(compute_iou(medsam_seg, gt3D[0][0].detach().cpu().numpy()), 4))
            dice_list.append(round(compute_dice(gt3D[0][0].detach().cpu().numpy().astype(np.uint8), medsam_seg), 4))
            accuracy_list.append((out_seg.argmax(1)==(gt_mask_class-1)).detach().cpu().numpy().astype(np.uint8)[0])
            
    return pred_list, click_points, click_labels, iou_list, dice_list,class_list,accuracy_list
...
if __name__ == "__main__":    
    ...
    if(args.dim==3):
        patch_size = 4
        in_channels = 816*8*8*8
        num_classes = 50
        num_prototype = 15
        mask_proto = nn.ModuleDict()
        mask_proto.update({"patch_embed": PatchEmbed3D(kernel_size=(patch_size, patch_size, patch_size),
                                         stride=(patch_size, patch_size, patch_size),
                                         in_chans=48,
                                         embed_dim=48)})

        mask_proto.update({"blocks":nn.ModuleList()})
        for i in range(2):
            block = Block3D(
                dim=384+384+48,
                num_heads=12,
                ...
                qkv_bias=True,
                window_size=8,
                input_size=(32 // patch_size, 32 // patch_size, 32 // patch_size),
            ).to(device)
            mask_proto["blocks"].append(block)

        mask_proto.update({"feat_norm": nn.LayerNorm(in_channels)})
        mask_proto.update({"mask_norm": nn.LayerNorm(num_classes)})
        mask_proto.to(device)
        sam_model_tune = sam_model_registry3D[args.model_type](checkpoint=None).to(device)
        if checkpoint_path is not None:
            model_dict = torch.load(checkpoint_path, map_location=device)
            state_dict = model_dict['model_state_dict']
            mask_proto_state_dict = model_dict["mask_proto"]
            prototypes = model_dict["prototypes"]
            mask_proto.load_state_dict(mask_proto_state_dict)
    ...
    for batch_data in tqdm(test_dataloader):
        ...
        if(os.path.exists(pred_path)):
            ...
            if(args.dim==3):
                seg_mask_list, points, labels, iou_list, dice_list,class_list,accuracy_list = finetune_model_predict3D(
                    image3D, gt3D, sam_model_tune, mask_proto, prototypes, device=device, 
        ...
        all_accuracy_list +=max(accuracy_list)[0]
        out_accuracy[img_name] = class_list[0].detach().cpu().numpy()[0]

    print('Mean IoU : ', sum(all_iou_list)/len(all_iou_list))
    print('Mean Dice: ', sum(all_dice_list)/len(all_dice_list))
    print('Mean Accuracy: ', all_accuracy_list/len(all_dice_list))
    ...
    print("Save accuracy to", args.save_name.replace('.py','_accuracy.py'))
    with open(args.save_name.replace('.py','_accuracy.py'), 'w') as f:
        f.writelines(f'# accuracy: \t{all_accuracy_list/len(all_dice_list)}\n')
        f.writelines('accuracy_Ts = {')
        for k, v in out_accuracy.items():
            f.writelines(f'\'{str(k[0])}\': {v},\n')
        f.writelines('}')
